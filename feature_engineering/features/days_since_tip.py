...
class DaysSinceTip(StaticFeature):
    ...
    def _compute_feature(self):
        ...
        self.orders_tip = pd.merge(self.orders_tip, orders_tip_copy[['order_id', self._feature]], on='order_id',
                                   how='left')
...
class RelDaysSinceTip(DaysSinceTip):
    ...
    def _compute_feature(self):
        ...
        orders_tip_copy[self.feature] = np.where(orders_tip_copy['mean_days_tipped_orders'] != -1,
                                                 orders_tip_copy['days_since_tip'] - orders_tip_copy[
                                                     'mean_days_tipped_orders'],
                                                 np.nan)
        ...
        self.orders_tip = pd.merge(self.orders_tip, orders_tip_copy[['order_id', self.feature]], on='order_id',
                                   how='left')
